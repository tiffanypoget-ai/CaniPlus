...
// Cron : annule automatiquement les cours privés confirmés mais non payés depuis
// plus de 48h, pour libérer le créneau côté Tiffany.
//
// Logique :
//   - Cherche private_course_requests où :
//       status = 'confirmed'
//       payment_status = 'pending'
//       updated_at OR confirmed_at < now() - 48h
...
// Délai (en heures) avant annulation auto d'une demande confirmée non payée
...
serve(async (req) => {
  ...
  try {
    ...
    // On utilise updated_at comme proxy de "confirmé depuis" (mis à jour à la confirmation).
    ...
    for (const req of stale) {
      try {
        ...
        await supabase.from('notifications').insert({
          ...
          body: `Ton cours privé${slotLabel} a été automatiquement annulé : pas de paiement reçu dans les ${CANCEL_AFTER_HOURS}h. Tu peux refaire une demande quand tu veux.`,
        ...
        try {
          ...
          await supabase.functions.invoke('notify-admin', {
            body: {
              ...
              body: `${memberLabel} n'a pas payé son cours privé${slotLabel} dans les ${CANCEL_AFTER_HOURS}h. Le créneau est libéré.`,
