...
export default function ChangePasswordModal({ onClose }) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 430, background: '#fff', borderRadius: '24px 24px 0 0', zIndex: 201, padding: '0 20px calc(32px + env(safe-area-inset-bottom,0px))', animation: 'slideUp 0.3s cubic-bezier(0.32,0.72,0,1)', boxShadow: '0 -8px 40px rgba(0,0,0,0.15)', }}>
        ...
        {!success ? (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showNew ? 'text' : 'password'} value={newPwd} onChange={e => { setNewPwd(e.target.value); setError(null); }} placeholder="Min. 8 caractères" style={{ width: '100%', padding: '13px 48px 13px 14px', background: '#f4f6f8', border: '2px solid #e5e7eb', borderRadius: 12, fontSize: 15, color: '#1F1F20', boxSizing: 'border-box' }} />
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showConfirm ? 'text' : 'password'} value={confirmPwd} onChange={e => { setConfirmPwd(e.target.value); setError(null); }} placeholder="Répète le mot de passe" style={{ width: '100%', padding: '13px 48px 13px 14px', background: '#f4f6f8', border: `2px solid ${confirmPwd && newPwd !== confirmPwd ? '#fecaca' : '#e5e7eb'}`, borderRadius: 12, fontSize: 15, color: '#1F1F20', boxSizing: 'border-box' }} />
            ...
            <button onClick={handleSubmit} disabled={loading} style={{ width: '100%', marginTop: 20, background: loading ? '#93c5fd' : 'linear-gradient(135deg,#2BABE1,#1a8bbf)', color: '#fff', border: 'none', borderRadius: 16, padding: '16px', fontSize: 16, fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, }}>
              {loading ? <><div style={{ width: 18, height: 18, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />Mise à jour...</> : '🔒 Changer le mot de passe'}
            </button>
          </>
        ) : (
          /* Succès */
