...
// Permet Ã  un membre de changer son mot de passe via Supabase Auth
...
export default function ChangePasswordModal({ onClose }) {
  ...
  const strengthLabel = ['', 'Trop court', 'Correct', 'Fort ðª'][strength];
  const strengthColor = ['', '#ef4444', '#f59e0b', '#16a34a'][strength];

  const handleSubmit = async () => {
    if (newPwd.length < 8) { setError('Le mot de passe doit contenir au moins 8 caractÃ¨res.'); return; }
    if (newPwd !== confirmPwd) { setError('Les mots de passe ne correspondent pas.'); return; }
    setLoading(true); setError(null);
    const { error: e } = await supabase.auth.updateUser({ password: newPwd });
    setLoading(false);
    if (e) setError('Erreur lors du changement. Reconnecte-toi et rÃ©essaie.');
        ...
        {!success ? (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8, marginBottom: 22 }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#1F1F20' }}>ð Changer le mot de passe</div>
            ...
            >
              {loading
                ? <><div style={{ width: 18, height: 18, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />Mise Ã  jour...</>
                : 'ð Changer le mot de passe'}
            </button>
          </>
        ) : (
          /* SuccÃ¨s */
          <div style={{ textAlign: 'center', padding: '24px 0 8px' }}>
            <div style={{ width: 72, height: 72, background: '#dcfce7', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, margin: '0 auto 16px' }}>â</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: '#1F1F20', marginBottom: 8 }}>Mot de passe modifiÃ© !</div>
