...
export default function HomeScreen({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Tous les cours de la semaine + cours privés confirmés + news + chien en parallèle
      ...
      // Cours Privés confirmés avec chosen_slot dans la semaine courante
      const confirmedPrivate = (privateRes.data ?? []).filter(r => {
        const d = r.chosen_slot?.date ?? '';
        return d >= monday && d <= sunday;
      });

      // Construire liste unifiée triée par date+heure
      const unified = [
        ...groupCourses.map(c => ({
          key: `gc-${c.id}`,
          date: c.course_date,
