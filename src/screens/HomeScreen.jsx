...
export default function HomeScreen({ onNavigate }) {
  ...
  useEffect(() => {
    if (!profile) return;
    const load = async () => {
      // Inclure tous les statuts actifs (confirmed, planifie…) pour ne pas rater les cours privés admin
      ...
      const { data: newsData } = await supabase.from('news').select('id,title,content,created_at').eq('published', true).order('created_at', { ascending: false }).limit(1);
      if (newsData?.length) setLatestNews(newsData[0]);
      setLoading(false);
    };
    load();
  }, [profile]);
  ...
  const menuItems = [
    ...
    { id: 'news',       emoji: '📣', title: 'Actualités',                sub: 'Nouvelles du club',                 dark: false, blue: false },
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto' }} className="screen-content">
      ...
      <div style={{ padding: '24px 16px 0' }}>
        ...
        {/* Bannière dernière news */}
        {latestNews && (
          <div
            onClick={() => onNavigate('news')}
            style={{
              background: 'linear-gradient(135deg, #1a3a4a, #2BABE1)',
              borderRadius: 16, padding: '14px 16px', marginBottom: 20, cursor: 'pointer',
              display: 'flex', alignItems: 'center', gap: 12,
              boxShadow: '0 2px 12px rgba(43,171,225,0.2)',
            }}
          >
            <span style={{ fontSize: 24, flexShrink: 0 }}>📣</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 }}>Actualité</div>
              <div style={{ fontSize: 14, fontWeight: 800, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{latestNews.title}</div>
              {latestNews.content && (
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {latestNews.content.slice(0, 80)}
                </div>
              )}
            </div>
