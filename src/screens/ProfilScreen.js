      ...
      <div style={{ padding: '0 16px 16px' }}>
        ...
        {cotisation && cotisation.status !== 'paid' && (
          <div style={{ background: 'linear-gradient(135deg,#fffbeb,#fef3c7)', border: '1px solid #fde68a', borderRadius: 14, padding: '10px 14px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1, fontSize: 12, fontWeight: 600, color: '#92400e' }}>
              Ta cotisation {dogs.length > 1 ? `de CHF ${150 * dogs.length} (${dogs.length} chiens × CHF 150)` : 'de CHF 150'} est à régler directement auprès de l'entraîneur.
            </div>
          </div>
        )}
        ...
        <Row
          ...
          badge={cotisation?.status === 'paid'
            ...
            : (dogs.length > 1 ? `CHF ${150 * dogs.length}` : 'CHF 150')}
          badgeColor={cotisation?.status === 'paid' ? (cotisationCancelled ? '#d97706' : '#16a34a') : '#d97706'}
          badgeBg={cotisation?.status === 'paid' ? (cotisationCancelled ? '#fef3c7' : '#dcfce7') : '#fef3c7'}
        />
