...
// âââ Ãcran login admin âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
      ...
      <div style={{ background: '#fff', borderRadius: 20, padding: 32, width: '100%', maxWidth: 380, boxShadow: '0 8px 40px rgba(0,0,0,0.2)' }}>
        ...
        <form onSubmit={handleLogin}>
          <input
            ...
            autoFocus
          />
          {error && <div style={{ color: C.red, fontSize: 13, marginBottom: 10, textAlign: 'center' }}>â ï¸ {error}</div>}
          <button
            type="submit"
          ...
          >
            {loading ? 'VÃ©rificationâ¦' : 'AccÃ©der'}
          </button>
        </form>
      </div>
    </div>
...
// âââ Onglet Membres ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function MembresTab({ pwd }) {
  ...
  const fmtBirth = (year) => year ? `nÃ©${year < 2020 ? '' : 'e'} en ${year}` : '';
  ...
  if (loading) return <div style={{ padding: 32, textAlign: 'center', color: C.gray }}>Chargementâ¦</div>;

  return (
    <div>
      <input
        placeholder="ð Rechercher un membreâ¦"
        value={search}
        ...
        return (
          <div key={member.id} style={{ background: C.card, borderRadius: 14, padding: 14, marginBottom: 10, boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              <div style={{ width: 40, height: 40, background: C.grayBg, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>ðââï¸</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
                  <Badge color={coti?.status === 'paid' ? C.green : C.orange} bg={coti?.status === 'paid' ? C.greenBg : C.orangeBg}>
                    {coti?.status === 'paid' ? 'Cotisation â' : 'Cotisation en attente'}
                  </Badge>
                  {premium && <Badge color="#92400e" bg="#fef3c7">Premium â¨</Badge>}
                  {lesson?.lesson_date && <Badge color={C.blue} bg="#e0f4fd">ð¯ {fmtLesson(lesson.lesson_date)}</Badge>}
                </div>
              </div>
            </div>
            ...
            {memberDogs.length > 0 && (
              <div style={{ marginTop: 10 }}>
                ...
                >
                  ð {memberDogs.length} chien{memberDogs.length > 1 ? 's' : ''} {dogsExpanded ? 'â²' : 'â¼'}
                </button>
                {dogsExpanded && memberDogs.map(dog => (
                  <div key={dog.id} style={{ background: '#fffbeb', borderRadius: 8, padding: '8px 10px', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <span style={{ fontSize: 12, color: C.gray }}>{dog.breed ? ` Â· ${dog.breed}` : ''}{dog.sex ? ` Â· ${dog.sex === 'M' ? 'â' : 'â'}` : ''}{dog.birth_year ? ` Â· ${fmtBirth(dog.birth_year)}` : ''}</span>
                    </div>
                    <Badge color={dog.vaccinated ? C.green : C.orange} bg={dog.vaccinated ? C.greenBg : C.orangeBg}>
                      {dog.vaccinated ? 'â VaccinÃ©' : 'Ã vÃ©rifier'}
                    </Badge>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
              ...
              >
                {actionLoading === member.id + '_cotisation' ? 'â¦' : coti?.status === 'paid' ? 'â Annuler cotisation' : 'â Valider cotisation'}
              </button>
              <button
              ...
              >
                {actionLoading === member.id + '_premium' ? 'â¦' : premium ? 'â Retirer premium' : 'â¨ Activer premium'}
              </button>
              <button
              ...
              >
                ð {lesson?.lesson_date ? 'Modifier cours' : 'Planifier cours'}
              </button>
              {lesson && (
                <button
      ...
      {lessonTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, padding: 24 }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: C.dark, marginBottom: 4 }}>ð Cours privÃ©</div>
            ...
            <input placeholder="Ex: terrain B, apporter la laisseâ¦" value={lessonNotes} onChange={e => setLessonNotes(e.target.value)} style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid #e5e7eb', fontSize: 14, marginBottom: 20, boxSizing: 'border-box', outline: 'none' }} />
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleSaveLesson} disabled={lessonSaving || !lessonDate || !lessonTime} style={{ flex: 2, padding: '11px', borderRadius: 10, border: 'none', background: lessonSaving ? '#9ca3af' : C.blue, color: '#fff', fontSize: 14, fontWeight: 700, cursor: lessonSaving ? 'not-allowed' : 'pointer' }}>
                {lessonSaving ? 'Enregistrementâ¦' : 'â Confirmer le cours'}
              </button>
            </div>
          </div>
        </div>
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 24, width: '100%', maxWidth: 360 }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: C.dark, marginBottom: 8 }}>â ï¸ Confirmer la suppression</div>
            <div style={{ fontSize: 14, color: C.gray, marginBottom: 20 }}>
              Supprimer le cours privÃ© de <strong>{confirmDelete.name}</strong> ? Cette action est irrÃ©versible.
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
...
// âââ Onglet Paiements ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function PaiementsTab({ pwd }) {
  ...
  const typeLabel = { cotisation_annuelle: 'Cotisation annuelle', lecon_privee: 'LeÃ§on privÃ©e', premium_mensuel: 'Premium', cours_theorique: 'Cours thÃ©orique' };
  const fmtDate = (iso) => iso ? new Date(iso).toLocaleDateString('fr-CH', { day: 'numeric', month: 'short', year: 'numeric' }) : 'â';
    ...
    <div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        <div style={{ flex: 1, background: C.greenBg, borderRadius: 12, padding: '12px 16px' }}>
          ...
          <div style={{ fontSize: 12, color: C.green }}>Paiements confirmÃ©s</div>
        </div>
      ...
      {subscriptions.map(sub => (
        <div key={sub.id} style={{ background: C.card, borderRadius: 12, padding: '12px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: C.gray }}>{sub.user_email ?? 'â'} Â· {fmtDate(sub.created_at)}</div>
          </div>
          <div style={{ textAlign: 'right', flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: sub.status === 'paid' ? C.green : C.orange }}>{fmtAmount[sub.type] ?? 'â'}</div>
            <div style={{ fontSize: 11, color: C.gray }}>{sub.status === 'paid' ? 'PayÃ©' : 'En attente'}</div>
            <button
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 24, width: '100%', maxWidth: 360 }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: C.dark, marginBottom: 8 }}>â ï¸ Supprimer ce paiement ?</div>
            <div style={{ fontSize: 14, color: C.gray, marginBottom: 20 }}>
              <strong>{typeLabel[confirmDelete.type] ?? confirmDelete.type}</strong> â {fmtAmount[confirmDelete.type] ?? 'â'}<br />
              {fmtDate(confirmDelete.created_at)}<br />
              <span style={{ color: C.red, fontSize: 12 }}>Cette action est irrÃ©versible.</span>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setConfirmDelete(null)} style={{ flex: 1, padding: '11px', borderRadius: 10, border: 'none', background: C.grayBg, color: C.gray, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Annuler</button>
...
// âââ Onglet Demandes cours privÃ© âââââââââââââââââââââââââââââââââââââââââââââ
function DemandesTab({ pwd, onPendingCount }) {
  ...
  const cancel = async (req) => {
    if (!window.confirm(`Annuler le cours privÃ© confirmÃ© de ${req.profiles?.full_name ?? 'ce membre'} ?`)) return;
  ...
  const fmtSlot = (slot) => {
    ...
    return `${days[d.getDay()]} ${d.getDate()} ${months[d.getMonth()]} Â· ${slot.start}â${slot.end}`;
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        {[
          ['pending',   `â³ En attente${pendingCount > 0 ? ` (${pendingCount})` : ''}`],
          ['confirmed', 'â ConfirmÃ©es'],
          ['cancelled', `ð« AnnulÃ©es${cancelledCount > 0 ? ` (${cancelledCount})` : ''}`],
          ['rejected',  'â RefusÃ©es'],
          ['all',       'Tout'],
        ].map(([val, label]) => (
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', color: C.gray, padding: 32 }}>
          {filter === 'pending' ? 'Aucune demande en attente ð' : 'Aucune demande'}
        </div>
      )}
      ...
      {filtered.map(req => (
        <div key={req.id} style={{ background: C.card, borderRadius: 14, padding: 14, marginBottom: 12, boxShadow: '0 1px 6px rgba(0,0,0,0.06)', borderLeft: `4px solid ${req.status === 'pending' ? C.orange : req.status === 'confirmed' ? C.green : req.status === 'cancelled' ? C.red : '#d1d5db'}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <div style={{ width: 36, height: 36, background: C.grayBg, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>ðââï¸</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.dark }}>{req.profiles?.full_name ?? 'â'}</div>
              <div style={{ fontSize: 11, color: C.gray }}>{req.profiles?.email ?? ''} Â· {new Date(req.created_at).toLocaleDateString('fr-CH', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
            </div>
            <Badge
            ...
            >
              {req.status === 'pending' ? 'En attente' : req.status === 'confirmed' ? 'ConfirmÃ©' : req.status === 'cancelled' ? 'ð« AnnulÃ© par le membre' : 'RefusÃ©'}
            </Badge>
          </div>
          ...
          <div style={{ fontSize: 11, fontWeight: 700, color: C.gray, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>
            {req.status === 'pending' ? 'CrÃ©neaux proposÃ©s â cliquez pour confirmer :' : req.status === 'confirmed' ? 'CrÃ©neau confirmÃ© :' : 'CrÃ©neaux proposÃ©s :'}
          </div>
          ...
          {req.status === 'confirmed' && req.chosen_slot ? (
            <div>
              ...
              <button onClick={() => cancel(req)} disabled={!!actionLoading} style={{ width: '100%', padding: '9px', borderRadius: 8, border: 'none', background: C.redBg, color: C.red, fontSize: 12, fontWeight: 700, cursor: actionLoading ? 'not-allowed' : 'pointer', opacity: actionLoading === req.id + '_cancel' ? 0.6 : 1 }}>
                {actionLoading === req.id + '_cancel' ? 'â¦' : 'â Annuler ce cours'}
              </button>
            </div>
          ) : (
            (req.availability_slots ?? []).map((slot, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: C.grayBg, borderRadius: 9, padding: '9px 12px', marginBottom: 6, gap: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: C.dark }}>ð {fmtSlot(slot)}</span>
                {req.status === 'pending' && (
                  <button onClick={() => confirm(req, slot)} disabled={!!actionLoading} style={{ padding: '6px 14px', borderRadius: 8, border: 'none', background: C.blue, color: '#fff', fontSize: 12, fontWeight: 700, cursor: actionLoading ? 'not-allowed' : 'pointer', opacity: actionLoading === req.id + '_confirm' ? 0.6 : 1, flexShrink: 0, whiteSpace: 'nowrap' }}>
                    {actionLoading === req.id + '_confirm' ? 'â¦' : 'â Confirmer'}
                  </button>
                )}
              </div>
            ))
          ...
          {req.status === 'pending' && (
            <button onClick={() => reject(req)} disabled={!!actionLoading} style={{ width: '100%', marginTop: 8, padding: '9px', borderRadius: 8, border: 'none', background: C.redBg, color: C.red, fontSize: 12, fontWeight: 700, cursor: actionLoading ? 'not-allowed' : 'pointer', opacity: actionLoading === req.id + '_reject' ? 0.6 : 1 }}>
              {actionLoading === req.id + '_reject' ? 'â¦' : 'â Refuser la demande'}
            </button>
          )}
        </div>
      ))}
...
// âââ Onglet News ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function NewsTab({ pwd }) {
  ...
  return (
    <div>
      ...
      {newsList.length === 0 && (
        <div style={{ textAlign: 'center', color: C.gray, padding: 32 }}>Aucune actualitÃ© publiÃ©e</div>
      )}
        ...
        <div key={item.id} style={{ background: C.card, borderRadius: 14, padding: 14, marginBottom: 10, boxShadow: '0 1px 6px rgba(0,0,0,0.06)', borderLeft: `4px solid ${item.published ? C.blue : '#d1d5db'}` }}>
          ...
          {item.content && (
            <div style={{ fontSize: 13, color: '#374151', marginBottom: 10, lineHeight: 1.5 }}>
              {item.content.length > 120 ? item.content.slice(0, 120) + 'â¦' : item.content}
            </div>
          )}
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              {item.published ? 'ð Masquer' : 'ð Publier'}
            </button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, padding: 24, maxHeight: '90dvh', overflowY: 'auto' }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: C.dark, marginBottom: 20 }}>
              {editing === 'new' ? '+ Nouvelle actualitÃ©' : 'âï¸ Modifier'}
            </div>
            ...
            <textarea
              ...
              placeholder="Texte de l'actualitÃ©â¦"
              rows={5}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleSave} disabled={saving || !form.title.trim()} style={{ flex: 2, padding: '11px', borderRadius: 10, border: 'none', background: saving ? '#9ca3af' : C.blue, color: '#fff', fontSize: 14, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
                {saving ? 'Enregistrementâ¦' : 'â Enregistrer'}
              </button>
            </div>
          </div>
        </div>
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 24, width: '100%', maxWidth: 360 }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: C.dark, marginBottom: 8 }}>â ï¸ Supprimer cette actualitÃ© ?</div>
            <div style={{ fontSize: 14, color: C.gray, marginBottom: 20 }}>
              <strong>Â«{confirmDelete.title}Â»</strong><br />
              <span style={{ color: C.red, fontSize: 12 }}>Cette action est irrÃ©versible.</span>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setConfirmDelete(null)} style={{ flex: 1, padding: '11px', borderRadius: 10, border: 'none', background: C.grayBg, color: C.gray, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Annuler</button>
...
// âââ Onglet Calendrier ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12, color: C.gray, marginTop: 8, textAlign: 'center' }}>
        Pour modifier ou supprimer un Ã©vÃ©nement, ouvrez-le directement dans Google Calendar.
      </div>
    </div>
  );
}
...
export default function AdminScreen() {
  ...
  const tabs = [
    { id: 'membres',    label: 'ð¥ Membres' },
    { id: 'paiements',  label: 'ð³ Paiements' },
    { id: 'demandes',   label: `ð Demandes${demandesBadge > 0 ? ` (${demandesBadge})` : ''}` },
    { id: 'news',       label: 'ð£ News' },
