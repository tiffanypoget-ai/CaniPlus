...
export default function LoginScreen() {
  ...
  const handleForgotPassword = async (e) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(resetEmail.trim().toLowerCase(), {
      redirectTo: 'https://cani-plus.vercel.app',
    });
        ...
        {!showForgot ? (
          ...
          <>
            ...
            <form onSubmit={handleLogin}>
              ...
              {error && (
                <div style={{ background: '#fee2e2', color: '#dc2626', padding: '12px 16px', borderRadius: 12, fontSize: 14, marginBottom: 16, fontWeight: 600 }}>⚠️ {error}</div>
              )}
              ...
              <button type="submit" disabled={loading} style={{
                ...
                boxShadow: '0 8px 24px rgba(43,171,225,0.35)', transition: 'opacity 0.2s', border: 'none',
              }}>
            ...
            {!resetSent ? (
                ...
                <form onSubmit={handleForgotPassword}>
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <input
                      ...
                      onFocus={() => setResetFocused(true)} onBlur={() => setResetFocused(false)}
                      placeholder="votre@email.com"
                      autoFocus
                      style={{ width: '100%', padding: '14px 16px', background: resetFocused ? '#e8f7fd' : '#f4f6f8', border: `2px solid ${resetFocused ? '#2BABE1' : '#e5e7eb'}`, borderRadius: 14, fontSize: 15, color: '#1F1F20', transition: 'border-color 0.2s, background 0.2s', boxSizing: 'border-box' }}
                    />
                  </div>

                  {resetError && (
                    <div style={{ background: '#fee2e2', color: '#dc2626', padding: '12px 16px', borderRadius: 12, fontSize: 14, marginBottom: 16, fontWeight: 600 }}>⚠️ {resetError}</div>
                  )}
                  ...
                  <button type="submit" disabled={resetLoading} style={{
                    ...
                    boxShadow: '0 8px 24px rgba(43,171,225,0.35)', border: 'none',
                  }}>
              ...
              <div style={{ textAlign: 'center', paddingTop: 20 }}>
                ...
                <p style={{ fontSize: 14, color: '#6b7280', lineHeight: 1.6, marginBottom: 24 }}>
                  Si l'adresse <strong>{resetEmail}</strong> est associée à un compte, vous recevrez un lien de réinitialisation dans quelques minutes.
                </p>
